 ...
 * @desc Interval join
 */
public class JoinDouble_IntervalJoinCase {
    ...
    public static void main(String[] args) throws Exception {
        ...
        //TODO transformation
        SingleOutputStreamOperator<FactOrderItem> result = goodsWatermarkDS.keyBy(Goods::getGoodsId)
                ...
                .process(new ProcessJoinFunction<Goods, OrderItem, FactOrderItem>() {
                    ...
                    public void processElement(Goods left, OrderItem right, Context ctx, Collector<FactOrderItem> out) throws Exception {
                        ...
                        factOrderItem.setGoodsId(left.getGoodsId());
                        factOrderItem.setGoodsName(left.goodsName);
                        factOrderItem.setCount(new BigDecimal(right.count));
                        factOrderItem.setTotalMoney(new BigDecimal(right.count).multiply(left.goodsPrice));
    ...
    public static class GoodsSource extends RichSourceFunction<Goods> {
        ...
        public void run(SourceContext ctx) throws Exception {
            while (!isCancel){
                Goods.GOODS_LIST.stream().forEach(goods -> ctx.collect(goods));
